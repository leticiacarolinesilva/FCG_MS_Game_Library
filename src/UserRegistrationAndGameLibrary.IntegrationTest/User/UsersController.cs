...
public class UsersController : BaseIntegrationTests
{
    ...
    [Theory]
    [InlineData("Test User", "not-an-email", "ValidPass1!", "ValidPass1!")]
    [InlineData("Test User", "test@test.com", "not-an-password", "ValidPass1!")]
    [InlineData("", "test@test.com", "ValidPass1!", "ValidPass1!")]
    [InlineData("Test User", "test@test.com", "ValidPass1!", "ValidPass1")]
    public async Task RegisterUser_ShouldReturnBadRequest_WhenNameOrEmailOrPasswordAreInvalid(
        string name,
        string email,
        string password,
    ...
    public async Task RegisterUser_ShouldReturnBadRequest_WhenThereIsAlreadyUser()
    {
        var request = new RegisterUserDto
        {
            ...
            Password = "ValidPass1!",
            ConfirmationPassword = "ValidPass1!",
            Permission = Domain.Enums.AuthorizationPermissions.Admin
        };
